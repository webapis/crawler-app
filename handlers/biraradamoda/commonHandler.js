...
async function commonHandler({page,context,productPageSelector, linkSelector, linksToRemove, hostname}){
    ...
    if(start){
        ...
        debugger
        const links = await page.evaluate((linkSelector)=>Array.from( document.querySelectorAll(linkSelector)).map(m=>{return {href:m.href,title:m.innerText.replaceAll('\n','').trim()}}).filter(f=>f.href.includes(hostname)),linkSelector ) 
